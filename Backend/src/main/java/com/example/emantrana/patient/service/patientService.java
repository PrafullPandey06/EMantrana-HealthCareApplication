...
public class patientService {
    ...
    private final String folderpath = "C:\\Users\\prafull\\Desktop\\had_frontend\\had_project\\public\\assests\\";
    ...
    public patientRegisterResponseDTO registerPatient(registerPatientDTO request) {
        ...
        patient.setPhoneNumber(passwordEncoder.encode(request.getPhoneNumber())); // change phone number is encoded
        Patient savedPatient = patientRepo.save(patient);
        patientRegisterResponseDTO response = modelMapper.map(savedPatient, patientRegisterResponseDTO.class);
        return response;
    }
//loginResponseDTO -here changed from void
    public void verifyPatient(loginPatientDTO request) {
        // Patient patient = patientRepo.findByPhoneNumber(request.getPhoneNumber());
        // changed here
        Patient patient = patientRepo.findByPhoneNumber(passwordEncoder.encode(request.getPassword())); // this is changed
...
//         return response;
    ...
    public Patient findPatientByPhoneNumber(String phoneNumber) {
       ...
       // Patient patient = patientRepo.findByPhoneNumber(phoneNumber);
        Patient patient = patientRepo.findByPhoneNumber(passwordEncoder.encode(phoneNumber));
        if(patient == null) {
            throw new RuntimeException("Patient not found");
        }
    ...
    public String updatePatientPassword(updatePasswordDTO request) {
        // change here
        //Patient patient = patientRepo.findByPhoneNumber(request.getPhoneNumber());
        Patient patient = patientRepo.findByPhoneNumber(passwordEncoder.encode(request.getPhoneNumber()));
        if(patient == null) {
            throw new RuntimeException("Patient not found");
        }
        patient.setPassword(passwordEncoder.encode(request.getPassword()));
    ...
    public void updateProfile(UpdateProfileDTO request, Patient patient) {
       ...
       if(request.getFname() != "") {
           patient.setFname(request.getFname());
       }
       if(request.getLname() != "") {
           patient.setLname(request.getLname());
       }
       if(request.getEmail() != "") {
    ...
    //    p.setId(patient.getId());
       patientRepo.save(patient);
//        System.out.println(request.getFname());
//        System.out.println(request.getLname());
